 ...
 * NOT: Bu fonksiyon sadece UX için kullanılmalıdır (hızlı yönlendirme)
 * Gerçek güvenlik kontrolü server-side middleware'de yapılmaktadır
 * Free-trial kullanıcıları sadece belirli etkinliklere erişebilir
 */

// Free-trial kullanıcılarının erişebileceği sayfalar
// Sadece bu etkinliklere ve free-trial sayfasına erişebilirler
    ...
    if (freeTrialStarted && freeTrialEndDate) {
      ...
      if (endDate > now) {
        ...
        const isAllowed = FREE_TRIAL_ALLOWED_PATHS.some((path) => {
          // Tam eşleşme veya alt path (ör: /genel/bolum1/exercises/xyz)
